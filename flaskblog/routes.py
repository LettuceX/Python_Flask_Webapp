...
def register():
    ...
    if form.validate_on_submit(): #*if the form is valid, then route responses
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf-8')
        user = User(username=form.username.data, email=form.email.data, password=hashed_password)
        db.session.add(user)
        db.session.commit()
        flash(f'Your account has been created!','success') #success is the category we pass into the template
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Login Unsuccessful. Please check your login informations', 'danger')            
...
@app.route("/post/<int:post_id>") #variable post_id typechecked with int
def post(post_id):
...
def reset_token(token):
    ...
    if form.validate_on_submit(): #*if the form is valid, then route responses
        ...
        db.session.commit()
        flash(f'Your password has been updated! You are now able to log in','success') #success is the category we pass into the template
        return redirect(url_for('login'))
